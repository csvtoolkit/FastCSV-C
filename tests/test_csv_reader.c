...
void test_csv_reader_bom() {
    printf("Testing CSV reader with BOM...\n");

    // UTF-8 BOM (EF BB BF) followed by CSV content
    const unsigned char bom_content[] = {
        0xEF, 0xBB, 0xBF,
        'N','a','m','e',',','A','g','e','\n',
        'J','o','h','n',',','2','5','\n',
        'J','a','n','e',',','3','0','\n'
    };
    FILE *file = fopen("test_bom.csv", "wb");
    assert(file != NULL);
    assert(fwrite(bom_content, 1, sizeof(bom_content), file) == sizeof(bom_content));
    fclose(file);

    Arena arena;
    assert(arena_create(&arena, 4096) == ARENA_OK);
    CSVConfig *config = csv_config_create(&arena);
    csv_config_set_path(config, "test_bom.csv");
    csv_config_set_has_header(config, true);

    CSVReader *reader = csv_reader_init_standalone(config);
    assert(reader != NULL);
    assert(reader->headers_loaded);
    assert(reader->cached_header_count == 2);
    assert(strcmp(reader->cached_headers[0], "Name") == 0);
    assert(strcmp(reader->cached_headers[1], "Age") == 0);

    CSVRecord *record1 = csv_reader_next_record(reader);
    assert(record1 != NULL);
    assert(record1->field_count == 2);
    assert(strcmp(record1->fields[0], "John") == 0);
    assert(strcmp(record1->fields[1], "25") == 0);
    ...
    // Test rewind still works correctly with BOM
    csv_reader_rewind(reader);
    CSVRecord *after_rewind = csv_reader_next_record(reader);
    assert(after_rewind != NULL);
    assert(strcmp(after_rewind->fields[0], "John") == 0);

    csv_reader_free(reader);
    arena_destroy(&arena);
    remove("test_bom.csv");
    printf("✓ CSV reader BOM test passed\n");
}
